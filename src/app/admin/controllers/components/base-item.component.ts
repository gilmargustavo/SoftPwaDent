...
export abstract class BaseItemComponent<T> implements OnInit {
  ...
  constructor(protected snackBar: MdSnackBar, 
    ...
    protected validationService: ValidationService, protected afoDatabase: AngularFireOfflineDatabase
  ) {
  ...
  public ngOnInit() {
    this.items = this.afoDatabase.list('/' + this.componentName);
  ...
  public getItem() {
    ...
    this.isLoading = true;
    console.log("el component es:" + this.componentName);
    this.baseApi.getById(this.id, this.componentName)
      .subscribe((item) => {
        console.log(item);
  ...
  public populateDataForNewItem() {
    ...
    this.item[this.baseApi.keyName] = fechaActual;
  ...
  public onSubmit() {
    ...
    if (this.myForm.valid) {
      ...
      var actionButtonLabel='OK';
      this.busyIndicatorService.show();
      this.isSaving = true;
      this.baseApi.save(this.id, this.item, this.isItemEdited, this.componentName)
        .subscribe((item) => {
          this.isSaving = false;
          this.busyIndicatorService.hide();
          if (this.componentName == 'patoPersonales') {
          this.snackBar.open('Antecedentes patológicos guardados', actionButtonLabel, config);
          this.router.navigate(['../paciente'], { relativeTo: this.activatedRoute.parent });
          }
          if (this.componentName == 'patoFamiliares') {
            this.router.navigate(['../paciente'], { relativeTo: this.activatedRoute.parent });
          }
          if (this.componentName == 'pacientes') {
            this.router.navigate(['./'], { relativeTo: this.activatedRoute.parent });
          }
          if (this.componentName == 'citas') {
            this.snackBar.open('Antecedentes patológicos guardados', actionButtonLabel, config);
            this.router.navigate(['./'], { relativeTo: this.activatedRoute.parent });
          }
          if (this.componentName == 'doctores') {
            this.snackBar.open('Antecedentes patológicos guardados', actionButtonLabel, config);
            this.router.navigate(['./'], { relativeTo: this.activatedRoute.parent });
          }
          if (this.componentName == 'historial') {
            this.snackBar.open('Antecedentes patológicos guardados', actionButtonLabel, config);
            this.router.navigate(['./'], { relativeTo: this.activatedRoute.parent });
          }
          if (this.componentName == 'usuarios') {
            this.snackBar.open('Antecedentes patológicos guardados', actionButtonLabel, config);
            this.router.navigate(['./'], { relativeTo: this.activatedRoute.parent });
          }
  ...
  public setItemId() {
    if (this.activatedRoute.snapshot.params['$key'] === 'new')
      this.id = null;
    else
      this.id = this.activatedRoute.snapshot.params['$key'];
    console.log("mi codigo es " + this.id);
  }
