...
export abstract class BaseApi<T> implements IApi<T> {
  ...
  constructor(protected http: Http, afoDatabase: AngularFireOfflineDatabase) {
    this.defaultHeaders.append('Access-Control-Allow-Origin', '*');
    this.datos= afoDatabase.object('/pacientes/1') as AfoObjectObservable<any[]>;
    this.datos1= afoDatabase.list('/pacientes/1'); 
  }
  ...
  public getById(id: string, select?: string): Rx.Observable<any> {
  ...
  public delete(id: string, ifMatch?: string): Rx.Observable<{}> {
    const path = `${this.basePath}/${this.resourceName}(${id})`;
  ...
  public save(id:string, item?: T, isEdited: boolean = true, doc?:string,extraHttpRequestParams?: any): Rx.Observable<T> {
    ...
    if (item[this.keyName] !== null) {
      console.log("MOdificaste");
      //return this.patch(item[this.keyName], item);
      return this.datos1.update(item[this.keyName],item);
    } else {
